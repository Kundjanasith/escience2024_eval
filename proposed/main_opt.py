...
def simulation(episode):
    ...
    for sim_time in range(SIMULATION_TIME):
        ...
        e_tem = time.time()
        print('===========',network.readableNanoSeconds(sim_time),e_tem-s_tem,len(network.successTransfer))
        if len(network.successTransfer) == len(packets): 
        # if len(network.successTransfer) == 20: 
            print('SUCCESS')
            break
    # AFTER SIMULATION
    print('----------------------------------')
    for s in network.switches.keys():
        ...
        network.switches[s].states = np.array(network.switches[s].states)
        network.switches[s].states = network.switches[s].states.reshape((network.switches[s].states.shape[0]*network.switches[s].states.shape[1],network.switches[s].states.shape[2]))
